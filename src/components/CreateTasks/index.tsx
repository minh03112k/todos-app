...
export default function CreateTasks() {
  ...
  return (
    <div className='flex min-h-screen flex-col p-24 items-center gap-8 item'>
      ...
      <div className="flex flex-row w-full">
        {
          Array.from({ length: 3 }, (_, index) => (
            <div key={index}
                 className='w-full flex items-center flex-col border-2 border-gray-400 p-8 mx-4 rounded-2xl shadow-lg shadow-gray-300'>
              <p
                className='items-center text-2xl font-bold'>{index === 0 ? 'To Do' : index === 1 ? 'In Progress' : 'Done'}</p>
              {blocks.filter((block) => block.position === index).map((block) => (
                <div key={block.id}
                     className='w-full border-1 border-amber-900 p-4 rounded-2xl flex items-center my-8 '>
